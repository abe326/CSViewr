...
1. ファイルの選択
   - メインCSVファイル：必須ファイルです
   - 連結CSVファイル：オプションファイルです
   - 複数のファイルを選択可能です

2. ファイルの削除
   - 各ファイルの右側にある × ボタンをクリックすることで個別に削除できます
   - ファイルを削除しても、同じファイルを再度選択することが可能です
   - 削除したファイルと同名のファイルでも、内容が更新されていれば新しい内容で読み込まれます

3. ファイル操作の特徴
   - ファイルの追加：既存のデータに追加される形で統合されます
   - ファイルの削除：該当するファイルのデータのみが削除されます
   - ファイルの再選択：最新の内容で再読み込みされます

### 注意事項

- メインCSVファイルを全て削除すると、連結CSVファイルのデータも表示されなくなります
- ファイルを削除後、同じファイルを選択する場合は、ファイルの内容が正しいことを確認してください
- 大量のデータを含むファイルの場合、読み込みに時間がかかる場合があります 

## フィルター機能

### 列ごとのフィルター

各列には2種類のフィルター入力欄があります：
...
1. 特定の文字列を含むデータの表示
   - 「含む」欄に検索したい文字列を入力
   - 例：部署列で "営業" と入力すると、部署名に "営業" を含む行のみ表示

2. 特定の文字列を含むデータの除外
   - 「除外」欄に除外したい文字列を入力
   - 例：ステータス列で "休職" と入力すると、ステータスに "休職" を含む行を除外

3. フィルターの組み合わせ
   - 複数の列で同時にフィルターを適用可能
   - 含むフィルターと除外フィルターを組み合わせて使用可能
   - 例：部署で "営業" を含み、ステータスで "休職" を除外

### 注意事項

- フィルターは即時適用されます
- 全文検索と列フィルターは併用可能です
- 除外フィルターは赤色で表示され、通常のフィルターと区別できます
- 複数の条件を組み合わせる場合、全ての条件を満たすデータのみが表示されます 